...
class MLP:
    ...
    def train(self, X, y, epoch=100, lr=0.2):
        for e in tqdm(range(epoch)):
            ...
            self.loss.append(self.cross_entropy(y_pred, y) + self.lbd * (np.sum(self.W1 ** 2) + np.sum(self.W2 ** 2)))
            dy = y_pred - y
            ...
            self.W2 = (1 - lr * self.lbd) * self.W2 - lr * dW2
            self.W1 = (1 - lr * self.lbd) * self.W1 - lr * dW1
            self.b2 -= lr * db2
            self.b1 -= lr * db1
...
model.plot_loss('L2.png', model.test(test_data, test_label_one_hot))
