...
class MLP:
    def __init__(self, input_size, output_size):
        ...
        self.W1 = np.random.normal(loc=0.0, scale=np.sqrt(2.0 / (input_size + 256.0)), size=(input_size, 256))
        self.W2 = np.random.normal(loc=0.0, scale=np.sqrt(2.0 / (256.0 + output_size)), size=(256, output_size))
    ...
    def train(self, X, y, epoch=100, lr=0.2):
        for e in tqdm(range(epoch)):
            ...
            self.loss.append(self.cross_entropy(y_pred, y))
            dy = y_pred - y
...
model.plot_loss('xavier_init.png', model.test(test_data, test_label_one_hot))
